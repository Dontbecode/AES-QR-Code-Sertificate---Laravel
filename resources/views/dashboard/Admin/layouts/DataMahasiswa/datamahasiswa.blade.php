...
<div class="col-md-12">
    <div class="card">
        <div class="header">
            ...
            <a href="{{ route('TambahMahasiswa') }}" class="d-none mr-4 btn btn-sm btn-primary btn-fill pull-right"><i
            ...
            <table class="table table-hover table-striped">
                ...
                @endphp
                  @foreach ($dtmahasiswa as $mahasiswa)
                    <tr>
                        <td>{{ $no++ }}</td>
                        <td>{{ $mahasiswa->No_peserta }}</td>
                        <td>{{ $mahasiswa->NIM }}</td>
                        <td>{{ $mahasiswa->Nama_mhs }}</td>
                        <td>{{ $mahasiswa->J_Kelamin }}</td>
                        <td>{{ $mahasiswa->ProgramStudi }}</td>
                        <td>{{ $mahasiswa->Angkatan }}</td>
                        <td>{{ $mahasiswa->No_tlpn }}</td>
                        <td>{{ $mahasiswa->email }}</td>
                        <td>
                            <a href="/EditMahasiswa/{{$mahasiswa->id }}" class="btn btn-warning btn-sm btn-fill">Edit</a>
                            <a href="#" class="btn btn-danger btn-sm btn-fill delete" data-id="{{ $mahasiswa->id }}" data-nama="{{ $mahasiswa->Nama_mhs }}" >Hapus</a>
                          </td>
                    </tr>
                    @endforeach
                </tbody>
  ...
  $('.delete').click( function(){
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
          window.location = "/HapusMahasiswa/"+jenisid+""
          Swal.fire(
            'Deleted!',
